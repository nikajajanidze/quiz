        ...
        <div class="row">
            ...
            <div class="col-md-4 m-auto">
                <div class="block mt-5">
                    ...
                    @php $answers = $questions[$i]->getAnswers() @endphp
                    <div class="block__content {{$i == 0 ? 'active' : ''}}">
                        <h2 class="block__title">{{$questions[$i]->title}}</h2>
                        <div class="block__question">
                            {!! $questions[$i]->text !!}
                        </div>
                        <div class="block__answers-container d-flex flex-column mt-3">
                           @foreach($answers as $answer)
                            <button class="btn-ans block__answer-type-2" type="button" data-toggle="modal" data-target="#confirmModal" data-question-id="{{$questions[$i]->id}}" data-answer-id="{{$answer->id}}">
                                {{ $answer->text }}
                            </button>
                            @endforeach
                        </div>
